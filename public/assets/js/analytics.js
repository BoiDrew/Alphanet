...
export function renderAnalytics(requests) {
  ...
  if (kpi) {
    kpi.innerHTML = `
      <div class="metric">
        <div class="metric-label">${t("kpiTotal")}</div>
        <div class="metric-val">${total}</div>
      </div>
      <div class="metric">
        <div class="metric-label">${t("kpiPending")}</div>
        <div class="metric-val">${pending}</div>
      </div>
      <div class="metric">
        <div class="metric-label">${t("kpiFulfilled")}</div>
        <div class="metric-val">${done}</div>
        <div class="metric-sub">${total ? Math.round(done/total*100) : 0}% ${t("kpiCompletion")}</div>
      </div>
      <div class="metric">
        <div class="metric-label">${t("kpiUrgent")}</div>
        <div class="metric-val">${urgent}</div>
      </div>
      <div class="metric">
        <div class="metric-label">${t("kpiSpend")}</div>
        <div class="metric-val">${spend.toFixed(0)} €</div>
      </div>
      <div class="metric">
        <div class="metric-label">${t("kpiSites")}</div>
  ...
  function byKey(key) {
    const map = {};
    requests.forEach(r => { const k = r[key] || "—"; map[k] = (map[k] || 0) + 1; });
  ...
  barChart("chart-products",
    Object.entries(prodMap).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([k,v])=>({k,v})),
    "teal"
  );
  ...
  barChart("chart-spend",
    Object.entries(spendMap).sort((a,b)=>b[1]-a[1]).map(([k,v])=>({k,v:Math.round(v)})),
    "teal"
  );
...
function renderExportTable(requests) {
  ...
  tbl.innerHTML = `
    <thead><tr>
      <th>${t("colDate")}</th><th>${t("colWorker")}</th><th>${t("colSite")}</th>
      <th>${t("colProduct")}</th><th>${t("colCategory")}</th><th>${t("colQty")}</th>
      <th>${t("colPriority")}</th><th>${t("colStatus")}</th><th>${t("colCost")}</th>
      <th>${t("colSupplier")}</th><th>${t("colFrequency")}</th><th>${t("colNotes")}</th>
    </tr></thead>
    <tbody>
      ${requests.map(r=>`<tr>
        <td>${r.date}</td>
...
function barChart(containerId, data, colorClass) {
  ...
  if (!el) return;
  if (!data.length) { el.innerHTML = `<div class="empty">${t("noData")}</div>`; return; }
